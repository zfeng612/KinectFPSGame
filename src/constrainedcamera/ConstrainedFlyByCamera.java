 ...
 *
 * @author Roochie
 * ConstrainedFlyByCamera (thx to Zarch for the name suggestion) fixes the wierd problem of being able to turn 360 degrees around.
 */
public class ConstrainedFlyByCamera implements AnalogListener, ActionListener {
    ...
    protected float maxLookUp = 0.9f;
    protected float maxLookDown = -0.9f;
     ...
     * @param cam
     */
    public ConstrainedFlyByCamera(Camera cam){
        this.cam = cam;
...
//                value = -value;
    ...
    public void onAnalog(String name, float value, float tpf) {
        ...
        if (name.equals("FLYCAM_Left")){
            ...
            if(cam.getDirection().getY()<maxLookUp){
                rotateCamera(-value * (invertY ? -1 : 1), cam.getLeft());
            }
        }else if (name.equals("FLYCAM_Down")){
            if(cam.getDirection().getY()>maxLookDown){
                rotateCamera(value * (invertY ? -1 : 1), cam.getLeft());
            }
    ...
    public float getMaxLookUp() {
        return maxLookUp;
    }

    public void setMaxLookUp(float maxLookUp) {
        this.maxLookUp = maxLookUp;
    }

    public float getMaxLookDown() {
        return maxLookDown;
    }

    public void setMaxLookDown(float maxLookDown) {
        this.maxLookDown = maxLookDown;
    }
}
